...
\begin{document}
% \maketitle (removed per requirement to drop first page)
\tableofcontents
\newpage
...
Figure1 shows the Use Case Diagram for the POS system. The two primary use cases required by the assignment are \textbf{Process Sale} and \textbf{Handle Returns}.
...
The system adopts a three-layer logical architecture. Figure2 presents the package/module organisation used in implementation, which helps separate UI, service logic, and domain model concerns.
...
Key business entities appear in the domain layer (Figure3). They are implemented in the \texttt{domain/} package and form the backbone of the application logic.
...
Team collaboration was fully tracked in GitHub. A condensed commit log covering all branches is embedded below.

% 添加截图
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{githistory.png}
  \caption{GitHub Commit History Overview}
\end{figure}
...
The complete source code, issue tracker, and pull-request history can be accessed on GitHub:

\begin{center}
\url{https://github.com/Sherlock0129/ProjectRobot1_2025}
\end{center}

\section{Demonstration Video}
A short walkthrough video showcasing the main features and workflows of the POS system is available at:

\begin{center}
\href{https://youtu.be/your-video-id}{https://youtu.be/your-video-id}
\end{center}
